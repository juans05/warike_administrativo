      ...
      {accounts.length === 0 ? (
        <section className="bg-white p-12 rounded-[3.5rem] shadow-sm border border-[var(--border)] text-center space-y-8 max-w-3xl mx-auto">
          ...
          <div className="space-y-4">
             ...
             <p className="text-xs font-bold text-gray-400">Puedes vincular <span className="text-[var(--primary)]">múltiples cuentas</span> si tu negocio tiene varias sedes o marcas.</p>
          </div>
          <button 
            onClick={handleConnect}
            disabled={isConnecting}
        ...
        <>
          ...
          <section className="space-y-4">
            <h3 className="text-xs font-black text-[var(--text-muted)] uppercase tracking-widest px-2">Cuentas Vinculadas ({accounts.length})</h3>
            <div className="flex flex-wrap gap-4">
              {accounts.map(account => (
                <div key={account.id} className={`bg-white px-6 py-4 rounded-2xl border-2 shadow-sm flex items-center gap-4 group transition-all ${
                  activeAccountFilter === account.id ? 'border-[var(--primary)] shadow-lg shadow-[var(--primary)]/10' : 'border-[var(--border)] hover:border-gray-300'
                }`}>
                  <button
                  ...
                  >
                    ...
                    <div className="text-left">
                      <p className="font-black text-sm text-[var(--text)]">{account.username}</p>
              ...
              <button
                onClick={handleConnect}
                disabled={isConnecting}
                className="px-6 py-4 rounded-2xl border-2 border-dashed border-[var(--primary)] text-[var(--primary)] font-black text-[10px] uppercase tracking-widest hover:bg-[var(--primary)] hover:text-white transition-all disabled:opacity-50 flex items-center gap-2"
              >
                + Agregar Cuenta
              </button>
            </div>
            {activeAccountFilter && (
              <button onClick={() => setActiveAccountFilter(null)} className="text-[10px] font-black text-[var(--primary)] uppercase tracking-widest hover:underline px-2">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
            ...
            <div className="lg:col-span-7 space-y-8">
              <div className="flex justify-between items-center">
                <h3 className="text-2xl font-black text-[var(--text)] font-warike">
              ...
              <div className="space-y-6">
                {comments.length === 0 ? (
                  <div className="bg-white p-12 rounded-[2.5rem] border border-[var(--border)] text-center space-y-4">
                    <div className="text-5xl">💬</div>
                    <p className="font-black text-[var(--text)]">Sin comentarios aún</p>
                    <p className="text-xs font-bold text-gray-400">Los comentarios de Instagram aparecerán aquí en tiempo real.</p>
                  </div>
                ) : (
                  comments.map(comment => (
                    <div key={comment.id} className="bg-white p-6 rounded-[2.5rem] border border-[var(--border)] shadow-sm space-y-4 group hover:shadow-lg transition-all">
                      <div className="flex gap-4">
                        ...
                        <div className="flex-1 space-y-1">
                          <div className="flex justify-between items-center">
                            <div className="flex items-center gap-2">
                              <span className="font-black text-sm text-[var(--text)]">{comment.authorUsername}</span>
                              {accounts.length > 1 && (
                      ...
                      {(comment.isReplied && (comment.aiReply || comment.manualReply)) ? (
                        <div className="ml-16 bg-[var(--background)] p-4 rounded-2xl border-l-4 border-[var(--primary)] space-y-2">
                           <div className="flex items-center gap-2">
                              <span className="text-[10px] bg-[var(--primary)]/10 text-[var(--primary)] px-2 py-0.5 rounded-full font-black uppercase tracking-widest">
                        ...
                        <div className="ml-16 flex gap-3 opacity-0 group-hover:opacity-100 transition-opacity">
                          ...
                          <button
                            ...
                            className="bg-[var(--text)] text-white px-6 py-2 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-[var(--primary)] transition-colors"
                          >
                            Enviar
                          </button>
                        </div>
...
function ToggleSetting({ title, desc, checked, onChange }: { title: string, desc: string, checked: boolean, onChange: (v: boolean) => void }) {
  return (
    <div className="flex justify-between items-center p-5 bg-[var(--background)] rounded-3xl border border-[var(--border)] cursor-pointer hover:border-gray-300 transition-colors" onClick={() => onChange(!checked)}>
      <div className="flex-1 pr-4">
        <p className="font-black text-[var(--text)] text-sm">{title}</p>
        <p className="text-[10px] font-bold text-[var(--text-muted)] mt-1">{desc}</p>
      </div>
      <div className={`w-12 h-6 rounded-full relative transition-all shrink-0 ${checked ? 'bg-[var(--primary)]' : 'bg-gray-200'}`}>
